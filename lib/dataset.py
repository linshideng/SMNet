...
def augment(img_in, img_tar, flip_h=True, rot=True, noise=False):
...
class DatasetFromFolderLab(data.Dataset):
    def __init__(self, HR_dir, LR_dir, patch_size, upscale_factor, data_augmentation,transform=None):
        super(DatasetFromFolderLab, self).__init__()
        self.hr_image_filenames = [join(HR_dir, x) for x in listdir(HR_dir) if is_image_file(x)]
        self.lr_image_filenames = [join(LR_dir, x) for x in listdir(HR_dir) if is_image_file(x)]
        self.patch_size = patch_size
        self.upscale_factor = upscale_factor
    ...
    def __getitem__(self, index):
        ...
        if self.transform:  # to tensor,[0,1]
            img_in = self.transform(img_in)
            img_tar = self.transform(img_tar)
        # if to lab, the [0,1] will be about [-100,100]
        img_in = colors.rgb_to_lab(img_in)
        img_tar = colors.rgb_to_lab(img_tar)
        return img_in, img_tar
